...
Categories organize transactions into a two-level hierarchy: a primary category (e.g., `FOOD_AND_DRINK`) and a detailed subcategory (e.g., `FOOD_AND_DRINK_GROCERIES`). Breadbox ships with the Plaid taxonomy by default, and you can customize it by creating, editing, or merging categories. Transaction rules and manual overrides reference categories by their `slug`.
...
Returns an array of category objects. Primary categories have `primary_slug: null`; detailed subcategories carry the parent's slug. See the [Category response fields](#category-response-fields) reference at the bottom of this page.
...
Deletes a category. Transactions currently assigned to this category will have their category cleared unless you merge the category into another before deleting. You cannot delete a primary category that still has active detailed subcategories — that case returns `409 CATEGORY_UNDELETABLE`.
...
`affected_transactions` is the count of transactions that had their category cleared as a result of the delete.
...
POST /api/v1/categories/import?replace=true
```
...
<ParamField query="replace" type="boolean" default="false">
  When `true`, Breadbox replaces the category set entirely — categories not present in the uploaded TSV are deleted. Default merges into the existing set (upsert only).
</ParamField>
...
<ParamField path="id" type="string" required>
  The source category to merge and delete. Accepts UUID, short ID, or slug.
</ParamField>
...
```json
{
  "target_id": "food-and-drink-restaurants"
}
```

<ParamField body="target_id" type="string" required>
  The UUID, short ID, or slug of the target category that will receive all transactions from the source.
</ParamField>
...
curl -X POST \
  -H "X-API-Key: bb_your_key" \
  -H "Content-Type: application/json" \
  -d '{"target_id": "food-and-drink-restaurants"}' \
...
| Primary category still has active subcategories | `409` | `CATEGORY_UNDELETABLE` |
| Invalid request body | `400` | `VALIDATION_ERROR` |
| TSV import parse error | `400` | `IMPORT_ERROR` |