...
The REST surface is `GET/POST/PATCH/DELETE /api/v1/workflows/prompt-blocks` — see the [API Reference](/api/overview).

<Note>
  The agent REST endpoints now live under `/api/v1/workflows/*` (formerly `/api/v1/agents/*`). Update any saved scripts — the old paths no longer resolve.
</Note>
