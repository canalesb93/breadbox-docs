...
Write tools for editing transactions. `update_transactions` is the single primitive — it covers category, tags, and a comment in one atomic op per row, for up to 50 transactions per call.
