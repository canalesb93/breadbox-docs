...
// TODO: Let's leverage mintlify to better structure this page and we should validate each flow by deploying an agent that does some web research to ensure its up to date. Would also love to add integration/instructions to:\
- claude desktop via the UI (I can help adding images if you can setup the section for it)\
- openclaw setup (autonomous agent setup that runs in its own VM)\
- manus setup\
- chatgpt and/or codex (if possible)\
- gemini and/or gemini cli (if possible)
