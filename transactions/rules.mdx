...
<Warning>
  `assign_series` runs **at sync time only** in v1 — retroactive [apply](#applying-a-rule-retroactively) does not materialize it yet. Existing transactions join the series when they next sync, or you can link them from the dashboard or the `assign_series` MCP tool.
</Warning>
