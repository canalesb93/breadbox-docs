...
| `counterparty_short_id` | string | Short ID of the assigned [counterparty](/transactions/counterparties) — the canonical "other side" of the charge. `null` when none is bound. Prefers this over `merchant_name` for display when present. |
| `counterparty_name` | string | Canonical counterparty display name (e.g., `Amazon`). `null` when none is bound. |
| `counterparty_logo_url` | string | Resolved logo URL for the counterparty (manual override or logo.dev hotlink). `null` when none is bound or no logo is configured. |
