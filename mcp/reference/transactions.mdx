...
<Warning>
  **Deprecated.** Use [`list_annotations`](#list-annotations) with `kinds=['comment']` instead. The new tool returns the same comment data using the canonical annotation shape — note the field renames: `actor_type` / `actor_id` / `actor_name` (was `author_*`) and `payload.content` (was top-level `content`). `list_transaction_comments` will be removed in a future release.
</Warning>
