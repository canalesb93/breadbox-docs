...
- **Tag-based workflows** — tags allow flexible coordination between humans and agents (e.g., the seeded <Tooltip headline="needs-review tag" tip="Seeded tag used as the default review queue. A system rule auto-tags every new transaction on sync; removing the tag takes the row out of the queue." cta="Review workflow" href="/transactions/review-workflow">`needs-review`</Tooltip> tag lets an agent flag transactions for you to look at)
