...
```json
{
  "operations": [
    ...
    },
    {
      ...
      "category_slug": "general_merchandise",
      "tags_to_remove": [
...
- **Never touch <Tooltip headline="category_override" tip="Who last set the category: 'none' (rule-applied or unset), 'agent' (AI-set), or 'user' (manually set — protected from automatic changes).">`category_override = 'user'`</Tooltip> rows** for category changes. The `update_transactions` op will report the skip; trust that signal.
