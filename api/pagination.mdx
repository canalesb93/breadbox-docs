...
Large list endpoints in the Breadbox API (`GET /transactions` and `GET /rules`) use cursor-based pagination rather than offset-based pagination. You navigate through results by passing an opaque cursor token from one response into the next request, rather than specifying a page number or offset.
...
Smaller list endpoints (`GET /accounts`, `GET /categories`, `GET /connections`, `GET /users`, `GET /reports`) are **not** paginated — they return a top-level JSON array with every matching record.
...
<ResponseField name="transactions | rules" type="array" required>
