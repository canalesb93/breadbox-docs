...
Breadbox handles them with **Series** (also called Subscriptions): a series groups the repeating charges from one merchant so you can see the whole subscription as a single thing, track what it costs, and act on it in one place.

<Note>
  Series is in **Beta**. It's available in the admin dashboard under **Subscriptions**, over the REST API, and over MCP. The shapes below are stable, but expect the surface to keep growing.
</Note>
...
Candidates wait for a verdict. From the **Subscriptions** page you can:

- **Confirm** — yes, this is a subscription. The series becomes `active`.
- **Reject** — not a subscription. The rejection is *sticky*: the detector won't re-propose it at the same amount band.
...
<Note>
  In v1 the `assign_series` action runs **at sync time only** — retroactive *apply* doesn't back-fill it yet. Existing charges join when they next sync, or you can link them from the dashboard or the `assign_series` MCP tool.
</Note>
...
- **See the portfolio.** The **Subscriptions** page lists every active series with its cadence and expected amount.
