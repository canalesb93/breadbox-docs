...
<Note>
  Draft — verify with current Openclaw docs before publishing. Openclaw routes outbound messages through the same channels it manages for inbound traffic: Slack, Telegram, Discord, WhatsApp, iMessage (via BlueBubbles), and others. The exact `messages_send` call syntax and the channel routing config depend on how your Openclaw gateway is configured. See [https://docs.openclaw.ai](https://docs.openclaw.ai) for the current outbound messaging reference. The agent prompt below uses a generic `notify_human` placeholder that you should replace with the appropriate Openclaw channel call.
</Note>
...
4. Send the alert through Openclaw's configured notification channel
   (replace this instruction with the actual Openclaw messages_send
   call for your channel — Slack, Telegram, Discord, etc.).
