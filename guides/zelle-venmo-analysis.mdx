...
<Note>
  The agent can easily misattribute when a household member starts a new recurring relationship (a new babysitter, a new rent situation). Expect the first few transactions to a new counterparty to always land in `p2p-unmapped` — that's working as intended.
</Note>
