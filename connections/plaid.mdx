...
To remove a Plaid connection, click **Remove Connection** on the connection detail page and confirm the dialog. The same operations — list, status, manual sync — are also exposed via the [Connections API](/api/connections).
