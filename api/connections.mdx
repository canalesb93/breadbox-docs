...
Returns an array of connection objects. The example below contrasts a healthy `active` connection with one in the `error` state (carrying provider `error_code` and `error_message`). See the [Connection response fields](#connection-response-fields) reference at the bottom of this page.
...
Returns the connection object plus a nested `last_sync_log` with counts and timing from the most recent sync attempt. The `last_sync_log` object is `null` if no sync has ever been run for this connection.
